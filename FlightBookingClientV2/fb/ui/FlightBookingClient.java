	...
	private void loop() {
		while(true){
			...
			if (line.toUpperCase().startsWith(FlightBookingConstants.QUERY)) {
				// query <from,to,leave_time,back_time>  round-trip
				// query <from,to,leave_time> 	one-stop trip
				if(clientHOPP.checkInputOfQuery(losePrefix(line, FlightBookingConstants.QUERY)))
				...
				if(clientHOPP.checkInputOfReg(losePrefix(line, FlightBookingConstants.REG)))
				...
				if(clientHOPP.checkInputOfCheck(losePrefix(line, FlightBookingConstants.CHECK)))
