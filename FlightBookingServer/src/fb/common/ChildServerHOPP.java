...
public class ChildServerHOPP implements ServerSevice {
	...
	public List<Flight> queryRespFromQanDB(String input) {
		...
		if (city.length == 2) {
			...
			qanList1 =  dao.queryFlightListFromQan(city[0], city[1]);
			qanList2 =  dao.queryFlightListFromQan(city[1], city[0]);
	...
	public List<Flight> queryRespFromAcDB(String input) {
		...
		if (city.length == 2) {
			FlightDao dao = new FlightDaoImpl();
			//list = dao.queryFlightListFromAll(city[0], city[1]);
			acList1 =  dao.queryFlightListfromAC(city[0], city[1]);
			acList2 =  dao.queryFlightListfromAC(city[1], city[0]);
